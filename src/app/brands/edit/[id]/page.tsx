...
export default function Page(): React.JSX.Element {
  ...
  useEffect(() => {
    const fetchBrand = async () => {
      ...
      try {
        const response = await nestApiInstance.get(`/brands/${brandId}`);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      let response;
      if (brandId) {
        response = await nestApiInstance.put(`/brands/${brandId}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      } else {
        response = await nestApiInstance.post(`/brands/`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      }

      if (response.data.statusCode === 200) {
        alert(brandId ? 'Cập nhật thành công' : 'Thêm thương hiệu thành công');
        router.push('/brands');
      } else {
