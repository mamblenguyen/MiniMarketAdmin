...
const columnNames = ['Ảnh', 'Tên biến thể', 'Số lượng' , 'Thao tác'];
...
export default function Page(): React.JSX.Element {
  ...
  const fetchVariant = async () => {
    try {
      setLoading(true);
      const res = await nestApiInstance.get('/variant/');
  ...
  const handleDetail = (id: string, slug?: string) => {
    router.push(`/variant/view/${slug}`); // thay bằng route bạn muốn điều hướng tới
  };

  const handleClick = () => {
    router.push('/variant/add'); // thay bằng route bạn muốn điều hướng tới
  };
  ...
  const filteredBrands = variant.filter((variant) => variant.name.toLowerCase().includes(filterText.toLowerCase()));
